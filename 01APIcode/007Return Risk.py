...
filepath="E://01_api//Return_Risk.txt"
path="E://01_api//Return_Risk_1.txt"

url="http://120.77.57.67:8080/dataservice/v1/secuirties/HF00000008%2CHF0000000G/return-risk?timeRange=Last1Year&endDate=2017-06-01"
data=get_api(url)
data=json.loads(data)
date=data.get("data").get("categories")
value_HF00000008=data.get("data").get("dataset")[0]
value_HF00000008=value_HF00000008["map"]
value_HF0000000G=data.get("data").get("dataset")[1]
value_HF0000000G=value_HF0000000G["map"]
if len(value_HF00000008)==0:
    pass
else:
    for key,name in value_HF00000008.items():
        content="%s%s"%(key,name)
        writetxt(filepath,content)
        writetxt(filepath, "\n")
if len(value_HF00000008)==0:
    pass
for key,name in value_HF0000000G.items():
    content="%-25s%s"%(key,name)
    content=str(content)
    writetxt(filepath,content)
    writetxt(filepath, "\n")
